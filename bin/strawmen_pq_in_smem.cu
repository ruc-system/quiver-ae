...
// memory class of the per-slot PQ distance table inside the GPU kernel.
// Default Q=2 (the typical occupancy-cliff failure point); Q≥3 will trigger
// a clean "LAUNCH_ABORTED (smem overflow)" diagnostic — that failure IS the
// strawman's expected outcome and the binary still exits 0 so log scrapers
// can capture it.
...
int main(int argc, char** argv) {
  ...
  argparse::ArgumentParser program("strawmen_pq_in_smem", "",
    ...
    if (run_num_blocks.size() > 1) {
      std::cout << "\n"
                << shared::colorize("============================================================",
                                    shared::ansi_gray())
                << "\n"
                << shared::colorize("PQInSmem Sweep:", shared::ansi_cyan(), true)
                << " num_blocks=" << run_nb
