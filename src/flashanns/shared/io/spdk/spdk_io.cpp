...
class SpdkIOImpl : public IndexLoader {
 public:
  ...
  void submit_traced_task(
      const std::vector<TracedIoRequest>& req, int tid, int cid) override {
    io_stats_record_enqueued(stats_.get(), req.size());
    for (const auto& r : req) {
      ...
      int ssd = logical_block % num_ssd;
      long off = base_lba_ + logical_block / num_ssd;
      submit_queue[ssd]->push(Req(cid, off, r.dest, r.complete_ns), tid);
    }
    ready[cid].fetch_add(req.size());
  }
#endif
 ...
 private:
  ...
  struct Req {
    ...
    volatile int32_t* status_ptr;
    int32_t status_value;
    Req() : cid(0), r(0), dst(nullptr), status_ptr(nullptr), status_value(0) {}
    Req(int cid, long r, void *dst)
        : cid(cid), r(r), dst(dst), status_ptr(nullptr), status_value(0) {}
#ifdef FLASHANNS_LATENCY_PROBE
    Req(int cid, long r, void *dst, volatile int64_t* complete_ns)
        : cid(cid), r(r), dst(dst), status_ptr(nullptr), status_value(0),
  ...
  static void callback(void *ctx, const struct spdk_nvme_cpl *cpl) {
    ...
    if (cur->complete_ns != nullptr) {
      __atomic_store_n(cur->complete_ns,
  ...
  bool find_ready(int ns_id, int &cid, long &blk, void *&dest,
                  volatile int32_t*& sp, int32_t& sv
#ifdef FLASHANNS_LATENCY_PROBE
                  , volatile int64_t*& complete_ns
#endif
                  ) {
  ...
  void worker_thread() {
    ...
    auto runner = [&](int nid) {
      ...
      while (!finished.load()) {
        ...
        while (!idle_queue.empty() && find_ready(nid, tid, r, dst, sp, sv
#ifdef FLASHANNS_LATENCY_PROBE
                                                 , complete_ns
#endif
                                                 )) {
