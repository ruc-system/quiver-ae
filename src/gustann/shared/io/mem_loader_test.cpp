...
void require(bool condition, const char* message) {
  if (!condition) {
    std::cerr << "gustann_mem_loader_test failed: " << message << std::endl;
    std::exit(1);
  }
}
...
std::filesystem::path make_test_index() {
  const auto dir = std::filesystem::temp_directory_path() /
                   ("gustann_mem_loader_test_" + std::to_string(::getpid()));
  ...
  for (int page_id = 0; page_id < 3; ++page_id) {
    std::fill(page.begin(), page.end(), static_cast<uint8_t>(0x60 + page_id));
    out.write(reinterpret_cast<const char*>(page.data()), page.size());
  }
...
int main() {
  ...
  expect_page(heap_direct_page.data(), 0x62, "heap submit_direct page 2");
  ...
  expect_page(mmap_direct_page.data(), 0x62, "mmap submit_direct page 2");
